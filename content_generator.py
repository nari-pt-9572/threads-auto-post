...
BEFORE_EXAMPLES = """---
1年目の冬、彼女へのプレゼントを何日もかけて探してた。
3000円の予算で、ショッピングモールを何周もした。
見つけたものはどれも、少しだけ予算を超えてた。
結局、予算内のものを選んだ。
渡したとき、「かわいい」と笑ってくれた。
「なんでもいいよ、一緒にいたいだけ」と言われてた。
その言葉が本音だとわかってた。
だから余計、その笑顔が重かった。
...
「日帰りでよくない？」と提案した。
「いいよ」と返ってきた。
翌週の土日、日帰りで行った。砂浜を少し歩いて、夕方には帰った。
帰りの車の中で、彼女はずっと窓の外を見てた。
何も言わなかった。
泊まりたかったかどうかも、聞けなかった。
本当のことを言えなかった日が、また増えた。"""


AFTER_EXAMPLES = """---
彼女がずっと気になってると言ってた店を、誕生日に予約した。
コースで、ふたり合わせて3万を超えた。
席に着いたとき「いいの？こんなとこ」と聞かれた。
「大丈夫」と答えた。
少し間があって、「ありがとう」と言われた。
昔の僕は、この言葉を言わせるのが怖かった。
今日は、ただ嬉しかった。

---
旅行の計画を立てることになった。
「どこでもいいよ」と言ったら、彼女が少し驚いた顔をした。
しばらく黙ったあと、地図を開いて「ここ行きたかったんだ」と見せてくれた。
前は、行き先より先に予算を調べてた。宿代を見て、候補が消えてた。
今日は、行きたい場所が先に決まった。
そのことに気づいたのは、帰り道だった。"""
...
・具体的な金額を出す場合は「数百円のために〇〇した」「10円のために〇〇した」など極端な節約行動はNG。現実的な金額感で
・「〇〇円のために30キロ走った」など非現実的な行動はNG。あくまで日常でリアルに起こりうる場面だけ
...
AFTER_RULES = """・時系列は「理学療法士2年目以降・収入が変わり始めた頃〜今」の話のみ
・1年目・手取り22万・お金がなかった頃の話は絶対に出さない。before/afterを混ぜない
・収入の変化理由は絶対に説明しない
・具体的な金額を出す場合は現実的な範囲で。「数百円のために〇〇した」など極端な節約行動はNG
・物語・日記形式で書く。詩的な表現や抽象的な締めはNG
・昔との対比を自然に入れる（「昔は〜してた。今日は〜した。」）
...
6. 「数百円・数十円のために長距離移動」など非現実的な極端な節約行動が含まれていないか
