import os
from groq import Groq
from config import GROQ_API_KEY, POSTING_TOPIC


def load_persona() -> str:
    """persona.mdを読み込む"""
    persona_path = os.path.join(os.path.dirname(__file__), "persona.md")
    if os.path.exists(persona_path):
        with open(persona_path, "r", encoding="utf-8") as f:
            return f.read()
    return ""


def generate_post(strategy: dict, topic: str, time_slot: str = "morning") -> str:
    """
    PDCA戦略をもとにThreads投稿文を生成
    time_slot: "morning"(朝6時) or "evening"(夜21時)
    """
    client = Groq(api_key=GROQ_API_KEY)

    persona = load_persona()
    keywords = "、".join(strategy.get("keywords", []))
    direction = strategy.get("strategy", "")
    improvements = strategy.get("improvements", "")

    time_context = "朝6時（通勤・起床時間帯）" if time_slot == "morning" else "夜21時（帰宅後・リラックス時間帯）"

    persona_section = f"\n## ペルソナ・運用ガイドライン\n{persona}\n" if persona else ""

    prompt = f"""あなたは25歳の理学療法士(PT)として、Threadsに投稿するコンテンツクリエイターです。
{persona_section}
## 今回の投稿指示

PDCAから得た戦略:
- 今日の方向性: {direction}
- 改善ポイント: {improvements}
- 使うキーワード: {keywords}
- 時間帯: {time_context}

## 投稿文を1つ生成してください

### 必須ルール
- **140〜170文字**（絶対に守る）
- 一人称は「僕」（「俺」は使わない）
- ハッシュタグは使わない
- 絵文字は1〜2個まで
- ネガティブで終わらない（必ず希望を見せる）
- 1投稿=1テーマ（複数トピック詰め込みNG）

### 実際の数字（必ず守る・誇張禁止）
- 遠距離: 片道3時間 / 往復6時間（「11時間」などの誇張は絶対NG）
- 手取り: 22万円
- 副業収入: 月8〜10万
- 高速代: 片道約3,000円（往復6,000円）
- ガソリン代: 月約2万円

### 構成パターン（以下のどれかを使う）
パターンA: 具体的な場面描写 → 感情 → 現在の変化 → 「同じ人いる?」
パターンB: リアルな数字で現実を突きつける → 当時の感情 → 「でも今は違う」
パターンC: 日常のあるある1場面 → 「その気持ちわかる」→ 希望

### 「中身のある投稿」にするコツ
- 場面を1つに絞って具体的に描写する（「彼女に会いに行く途中、高速のSAで『財布大丈夫かな』って考えてた」など）
- 感情を正直に書く（「情けなかった」「しんどかった」「悔しかった」）
- 読んで「あ、わかる」と思える細部を入れる
- 抽象的なまとめ文（「でも今は大丈夫」）だけで終わらない

### 絶対NG
- 実際と違う数字・誇張（往復11時間など）
- PT技術・医療知識の話
- 副業の具体的な手法（暗示のみ）
- 彼女を悪者にする表現
- 「DM送って」などの押し付けがましい誘導
- ネガティブで終わる投稿
- 抽象的で薄い内容（「しんどかった。でも今は大丈夫」だけで終わるもの）

投稿文のみ返してください（説明・タイトル・注釈不要）:"""

    response = client.chat.completions.create(
        model="llama-3.3-70b-versatile",
        messages=[{"role": "user", "content": prompt}],
        max_tokens=400,
        temperature=0.8,
    )

    post_text = response.choices[0].message.content.strip()

    # 文字数チェック（170文字超えたらトリミング）
    if len(post_text) > 175:
        post_text = post_text[:170].rsplit("\n", 1)[0]

    return post_text
